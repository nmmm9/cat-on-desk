(() => {
  ...
  function shouldHook(url, method) {
    ...
    return /StreamGenerate/i.test(url);
  }

  function notifyDone(meta) {
    console.log('[catondesk] Gemini response done →', meta?.url);
    window.dispatchEvent(
      new CustomEvent('catondesk:ai-done', {
        detail: { site: 'Gemini', message: 'Gemini 응답 완료', ...(meta || {}) },
      })
    );
  }
  ...
  // XHR fallback (Gemini는 XHR + batchexecute 자주 사용)
  ...
  console.log('[catondesk] hooked Gemini fetch + XHR');
})();
